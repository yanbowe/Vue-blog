<template>
    <div>
      <ul class="list">
        <li class="article" enter-active-class="animated fadeInLeft" :class="{active: activeIndex === index, published: isPublished === 1}" v-for="{title,createTime,isPublished,isChosen},index in articleList" @click="select(index)">
          <header>{{title}}</header>
          <p>{{createTime}}</p>
        </li>
      </ul>
...
<style type="text/scss"  lang="scss" scoped>
  ...
  .active {
    border: 1px solid #32D2DB;
  }
  .published {
    border-right: 4px solid #32D2DB;
  }
